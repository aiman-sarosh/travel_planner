...
#     return OllamaLLM(model="llama3.1:8b", temperature=0.1)


def get_ollama_llm():
    """Get Ollama LLM instance (optionally using env config)"""
    
    # Optional: allow model name override via env
    model_name = "llama3.1:8b" #os.getenv("OLLAMA_MODEL", "llama3.1:8b")
    
    # Optional: check if Ollama host is set
    base_url = "http://localhost:11434" # os.getenv("OLLAMA_BASE_URL", "http://localhost:11434")
    ...
    try:
        ...
        print("Warning: Failed to initialize Ollama LLM")
        print(f"   Error: {e}")
